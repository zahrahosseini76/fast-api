...
class ParseResultMixin(object):
    def _generate_authority(self, attributes):
        ...
        groupinfo, host, port = (
            attributes[p] for p in ("groupinfo", "host", "port")
        )
        if (
            ...
            return normalizers.normalize_authority(
                (
                    compat.to_str(groupinfo, self.encoding),
...
class ParseResult(
    ...
    def from_parts(
        ...
        groupinfo, host, port = authority_from(uri_ref, strict=True)
        return cls(
    ...
    def from_string(
        ...
        groupinfo, host, port = authority_from(reference, strict)
    ...
    def copy_with(
        ...
        attributes = zip(
            PARSED_COMPONENTS,
            (scheme, groupinfo, host, port, path, query, fragment),
        )
        attrs_dict = {}
...
class ParseResultBytes(
    ...
    def from_parts(
        ...
        return cls(
            ...
            groupinfo=to_bytes(groupinfo, encoding),
...
def split_authority(authority):
    ...
    groupinfo = host = port = None
    # Initialize an extra var we may need to use
    extra_host = None
    # Set-up rest in case there is no groupinfo portion
    rest = authority

    if "@" in authority:
        groupinfo, rest = authority.rsplit("@", 1)
...
def authority_from(reference, strict):
    try:
        ...
        if strict:
            raise
        groupinfo, host, port = split_authority(reference.authority)
    else:
        # Thanks to Richard Barrell for this idea:
        # https://twitter.com/0x2ba22e11/status/617338811975139328
        groupinfo, host, port = (
            subauthority.get(p) for p in ("groupinfo", "host", "port")
        )
