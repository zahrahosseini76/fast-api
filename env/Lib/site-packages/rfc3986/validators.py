...
class Validator(object):
    ...
    def validate(self, uri):
        ...
        :raises PasswordForbidden:
            When a password is present in the groupinfo component but is
...
def check_password(uri):
    ...
    groupinfo = uri.groupinfo
    if not groupinfo:
        return
    credentials = groupinfo.split(":", 1)
...
_SUBAUTHORITY_VALIDATORS = set(["groupinfo", "host", "port"])


def subauthority_component_is_valid(uri, component):
    """Determine if the groupinfo, host, and port are valid."""
    try:
