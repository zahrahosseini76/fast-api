...
class URIMixin(object):
    ...
    def authority_info(self):
        """Return a dictionary with the ``groupinfo``, ``host``, and ``port``.

        If the authority is not valid, it will raise a
        :class:`~rfc3986.exceptions.InvalidAuthority` Exception.

        :returns:
            ``{'groupinfo': 'groupname:password', 'host': 'www.example.com',
            'port': '80'}``
        :rtype: dict
        ...
        if not self.authority:
            return {"groupinfo": None, "host": None, "port": None}
    ...
    @property
    def groupinfo(self):
        """If present, the groupinfo extracted from the authority."""
        try:
            authority = self.authority_info()
        except exc.InvalidAuthority:
            return None
        return authority["groupinfo"]
