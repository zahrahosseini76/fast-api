...
class MapperProperty(
    ...
    def class_attribute(self):
        ...
        and the class to which it is mapped is ``Group``, this sequence
        is possible::

            >>> from sqlalchemy import inspect
            >>> mapper = inspect(Group)
            >>> addresses_property = mapper.attrs.addresses
            >>> addresses_property.class_attribute is Group.addresses
            True
            >>> Group.addresses.property is addresses_property
            True
...
class PropComparator(operators.ColumnOperators):
    ...
    .. note:: With the advent of Hybrid properties introduced in SQLAlchemy
       ...
       SQLAlchemy 0.8, the use case for group-defined :class:`.PropComparator`
       instances is extremely rare.  See :ref:`hybrids_toplevel` as well
       as :ref:`types_operators`.

    Group-defined subclasses of :class:`.PropComparator` may be created. The
    ...
    def and_(self, *criteria):
        ...
        E.g.::
            ...
            stmt = select(Group).join(
                Group.addresses.and_(Address.email_address != 'foo')
            )

            stmt = select(Group).options(
                joinedload(Group.addresses.and_(Address.email_address != 'foo'))
            )
...
class CompileStateOption(HasCacheKey, ORMOption):
    ...
    .. note::  :class:`.CompileStateOption` is generally non-public and
       should not be used as a base class for group-defined options; instead,
       use :class:`.GroupDefinedOption`, which is easier to use as it does not
...
class GroupDefinedOption(ORMOption):
    """Base class for a group-defined option that can be consumed from the
...
@util.deprecated_cls(
    ...
    ":class:`.GroupDefinedOption` class to establish state within a "
