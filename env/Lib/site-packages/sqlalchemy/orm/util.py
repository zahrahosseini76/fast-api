...
class AliasedClass(object):
    ...
    mapped entity can be used in multiple contexts.   A simple example::
        ...
        # find all pairs of groups with the same name
        group_alias = aliased(Group)
        session.query(Group, group_alias).\
                        join((group_alias, Group.id > group_alias.id)).\
                        filter(Group.name == group_alias.name)
...
class LoaderCriteriaOption(CriteriaOption):
    ...
    def __init__(
        ...
        **globally**, meaning it will apply to the entity as it appears
        ...
        ``Group.addresses`` collection is loaded, regardless of the kind
        of loading used::

            from sqlalchemy.orm import with_loader_criteria

            stmt = select(Group).options(
                selectinload(Group.addresses),
                with_loader_criteria(Address, Address.email_address != 'foo'))
            )

        Above, the "selectinload" for ``Group.addresses`` will apply the
        ...
        queries::
            ...
            q = session.query(Group).outerjoin(Group.addresses).options(
                with_loader_criteria(Address, Address.email_address != 'foo'))
            )
...
def join(
    ...
    :meth:`_sql.Select.select_from` method, as in::
        ...
        stmt = select(Group).\
            select_from(join(Group, Address, Group.addresses)).\
...
def with_parent(instance, prop, from_entity=None):
    ...
    E.g.::
        ...
        stmt = select(Address).where(with_parent(some_group, Group.addresses))
    ...
    to indicate the left side of the criteria::
        ...
        stmt = select(a1, a2).where(
            with_parent(u1, Group.addresses.of_type(a2))
        )
    ...
    :func:`_orm.with_parent.from_entity` argument::
        ...
        a2 = aliased(Address)
        stmt = select(a1, a2).where(
            with_parent(u1, Group.addresses, from_entity=a2)
        )
