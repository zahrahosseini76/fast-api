...
class _UnboundLoad(Load):
    ...
    def _bind_loader(self, entities, current_path, context, raiseerr):
        ...
        not exact API)::
            ...
            Load(
                Group,
                (Group, Group.orders.property))
            Load(
                Group,
                (Group, Group.orders.property, Order, Order.items.property))
...
def contains_eager(loadopt, attr, alias=None):
    ...
    the desired rows, i.e.::
        ...
        sess.query(Order).\
                join(Order.group).\
                options(contains_eager(Order.group))

    The above query would join from the ``Order`` entity to its related
    ``Group`` entity, and the returned ``Order`` objects would have the
    ``Order.group`` attribute pre-populated.
    ...
    collection of parent objects may already have been loaded::
        ...
        sess.query(Group).\
            ...
            options(contains_eager(Group.addresses)).\
            populate_existing()
...
def load_only(loadopt, *attrs):
    ...
    Example - given a class ``Group``, load only the ``name`` and ``fullname``
    attributes::

        session.query(Group).options(load_only(Group.name, Group.fullname))

    Example - given a relationship ``Group.addresses -> Address``, specify
    subquery loading for the ``Group.addresses`` collection, but on each
    ``Address`` object load only the ``email_address`` attribute::

        session.query(Group).options(
                subqueryload(Group.addresses).load_only(Address.email_address)
        )
    ...
    specifically referred to using the :class:`_orm.Load` constructor::
        ...
        session.query(Group, Address).join(Group.addresses).options(
                    Load(Group).load_only(Group.name, Group.fullname),
...
def joinedload(loadopt, attr, innerjoin=None):
    ...
    examples::
        ...
        # joined-load the "orders" collection on "Group"
        query(Group).options(joinedload(Group.orders))
    ...
    :param innerjoin: if ``True``, indicates that the joined eager load should
     use an inner join instead of the default of left outer join::
        ...
        query(Order).options(joinedload(Order.group, innerjoin=True))
...
def subqueryload(loadopt, attr):
    ...
    examples::
        ...
        # subquery-load the "orders" collection on "Group"
        query(Group).options(subqueryload(Group.orders))
...
def selectinload(loadopt, attr):
    ...
    examples::
        ...
        # selectin-load the "orders" collection on "Group"
        query(Group).options(selectinload(Group.orders))
