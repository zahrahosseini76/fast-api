...
class Query(
    ...
    ORM, both those formulated by end-group query operations as well as by
    ...
    def __init__(self, entities, session=None):
        ...
        E.g.::
            ...
            q = Query([Group, Address], session=some_session)

        The above is equivalent to::

            q = some_session.query(Group, Address)
    ...
    def get(self, ident):
        ...
        E.g.::
            ...
            my_group = session.query(Group).get(5)
    ...
    def from_self(self, *entities):
        ...
        into a SELECT of itself.  Given a query such as::
            ...
            q = session.query(Group).filter(Group.name.like('e%'))

        Given the :meth:`_query.Query.from_self` version::

            q = session.query(Group).filter(Group.name.like('e%')).from_self()

        This query renders as:

        .. sourcecode:: sql

            SELECT anon_1.group_id AS anon_1_group_id,
                   anon_1.group_name AS anon_1_group_name
            FROM (SELECT "group".id AS group_id, "group".name AS group_name
            FROM "group"
            WHERE "group".name LIKE :name_1) AS anon_1

        There are lots of cases where :meth:`_query.Query.from_self`
        may be useful.
        A simple one is where above, we may want to apply a row LIMIT to
        the set of group objects we query against, and then apply additional
        joins against that row-limited set::

            q = session.query(Group).filter(Group.name.like('e%')).\
                limit(5).from_self().\
                join(Group.addresses).filter(Address.email.like('q%'))

        The above query joins to the ``Address`` entity but only against the
        first five results of the ``Group`` query:
        ...
        .. sourcecode:: sql
            ...
            FROM (SELECT "group".id AS group_id, "group".name AS group_name
            FROM "group"
            WHERE "group".name LIKE :name_1
             LIMIT :param_1) AS anon_1
            JOIN address ON anon_1.group_id = address.group_id
        ...
        **automatic aliasing** to the entities inside the subquery, when
        ...
        refer to the ``Group`` entity without any additional aliasing applied
        ...
        The ORDER BY against ``Group.name`` is aliased to be in terms of the
        inner subquery:
        ...
        .. sourcecode:: sql
            ...
            WHERE "group".name LIKE :name_1
             LIMIT :param_1) AS anon_1
            JOIN address ON anon_1.group_id = address.group_id
            WHERE address.email LIKE :email_1 ORDER BY anon_1.group_name
        ...
        **Changing the Entities**
        ...
        columns are being queried.   In our example, we want ``Group.id``
        ...
        **Looking out for Inner / Outer Columns**
        ...
        columns.   Below illustrates a join of ``Address`` to ``Group``,
        then a subquery, and then we'd like :func:`.contains_eager` to access
        the ``Group`` columns::

            q = session.query(Address).join(Address.group).\
                filter(Group.name.like('e%'))

            q = q.add_entity(Group).from_self().\
                options(contains_eager(Address.group))

        We use :meth:`_query.Query.add_entity` above **before** we call
        :meth:`_query.Query.from_self`
        so that the ``Group`` columns are present
        ...
        .. sourcecode:: sql
            ...
            SELECT anon_1.address_id AS anon_1_address_id,
                   ...
                   anon_1.address_group_id AS anon_1_address_group_id,
                   anon_1.group_id AS anon_1_group_id,
                   anon_1.group_name AS anon_1_group_name
            FROM (
                SELECT address.id AS address_id,
                address.email AS address_email,
                address.group_id AS address_group_id,
                "group".id AS group_id,
                "group".name AS group_name
            FROM address JOIN "group" ON "group".id = address.group_id
            WHERE "group".name LIKE :name_1) AS anon_1

        If we didn't call ``add_entity(Group)``, but still asked
        :func:`.contains_eager` to load the ``Group`` entity, it would be
        forced to add the table on the outside without the correct
        join criteria - note the ``anon1, "group"`` phrase at
        the end:
        ...
        .. sourcecode:: sql
            ...
            FROM (
                ...
                address.group_id AS address_group_id
            FROM address JOIN "group" ON "group".id = address.group_id
            WHERE "group".name LIKE :name_1) AS anon_1, "group"
    ...
    def with_entities(self, *entities):
        ...
        e.g.::
            ...
            # Groups, filtered on some arbitrary criterion
            # and then ordered by related email address
            q = session.query(Group).\
                        join(Group.address).\
                        filter(Group.name.like('%ed%')).\
                        order_by(Address.email)

            # given *only* Group.id==5, Address.email, and 'q', what
            # would the *next* Group in the result be ?
    ...
    def with_for_update(
        ...
        E.g.::
            ...
            q = sess.query(Group).populate_existing().with_for_update(nowait=True, of=Group)

        The above query on a PostgreSQL backend will render like::

            SELECT groups.id AS groups_id FROM groups FOR UPDATE OF groups NOWAIT
    ...
    def _filter_by_zero(self):
        ...
        # for filter_by() from the "from object", i.e., if the group
        ...
        # to query(), e.g. query(Group).from_self() is still in terms of Group,
        ...
        # groups should now used aliased() for that, but I can't entirely get
    ...
    def join(self, target, *props, **kwargs):
        ...
        **Simple Relationship Joins**
        ...
        Consider a mapping between two classes ``Group`` and ``Address``,
        with a relationship ``Group.addresses`` representing a collection
        of ``Address`` objects associated with each ``Group``.   The most
        common usage of :meth:`_query.Query.join`
        is to create a JOIN along this
        relationship, using the ``Group.addresses`` attribute as an indicator
        for how this should occur::

            q = session.query(Group).join(Group.addresses)

        Where above, the call to :meth:`_query.Query.join` along
        ``Group.addresses`` will result in SQL approximately equivalent to::

            SELECT group.id, group.name
            FROM group JOIN address ON group.id = address.group_id

        In the above example we refer to ``Group.addresses`` as passed to
        ...
        .. note:: as seen in the above example, **the order in which each
           ...
           ``Group``, then ``Item``, then ``Order``, in our chain of joins; in
        ...
        **Joins to a Target Entity or Selectable**
        ...
        natural foreign key relationship between two entities::
            ...
            q = session.query(Group).join(Address)
        ...
        **Joins to a Target with an ON Clause**
        ...
        a SQL expression as the ON clause is as follows::
            ...
            q = session.query(Group).join(Address, Group.id==Address.group_id)

        The above form may also use a relationship-bound attribute as the
        ON clause as well::

            q = session.query(Group).join(Address, Group.addresses)
        ...
        aliases set up using the :func:`~sqlalchemy.orm.aliased` function::
            ...
            q = session.query(Group).\
                    join(a1, Group.addresses).\
                    join(a2, Group.addresses).\
        ...
        equivalent to the one above would be::
            ...
            q = session.query(Group).\
                    join(Group.addresses.of_type(a1)).\
                    join(Group.addresses.of_type(a2)).\
                    filter(a1.email_address == 'ed@foo.com').\
                    filter(a2.email_address == 'ed@bar.com')
        ...
        **Augmenting Built-in ON Clauses**
        ...
        with the default criteria using AND::
            ...
            q = session.query(Group).join(
                Group.addresses.and_(Address.email_address != 'foo@bar.com')
            )
        ...
        **Joining to Tables and Subqueries**
        ...
        entity using :func:`_orm.aliased`::
            ...
            q = session.query(Group).join(
                Group.addresses.of_type(address_subq)
            )
        ...
        **Controlling what to Join From**
        ...
        the :meth:`_query.Query.select_from` method may be used::
            ...
            q = session.query(Address).select_from(Group).\
                            join(Group.addresses).\
                            filter(Group.name == 'ed')

        Which will produce SQL similar to::

            SELECT address.* FROM group
                JOIN address ON group.id=address.group_id
                WHERE group.name = :name_1
        ...
        * Joining on relationship names rather than attributes::
            ...
            session.query(Group).join("addresses")
          ...
          **Modern calling pattern**:  Use the actual relationship,
          e.g. ``Group.addresses`` in the above case::

              session.query(Group).join(Group.addresses)
        ...
        * Multiple joins in one call::
            ...
            session.query(Group).join("orders", "items")

            session.query(Group).join(Group.orders, Order.items)

            session.query(Group).join(
                (Order, Group.orders),
                (Item, Item.order_id == Order.id)
            )

            session.query(Group).join(Order, Item)
          ...
          **Modern calling pattern**:  Use relationship-bound attributes
          ...
          side should be::
            ...
            session.query(Group).join(Group.orders).join(
                Item, Item.order_id == Order.id)
    ...
    def select_entity_from(self, from_obj):
        ...
        Given a case for :func:`.aliased` such as selecting ``Group``
        objects from a SELECT statement::

            select_stmt = select(Group).where(Group.id == 7)
            group_alias = aliased(Group, select_stmt)

            q = session.query(group_alias).\
                filter(group_alias.name == 'ed')

        Above, we apply the ``group_alias`` object explicitly throughout the
        query.  When it's not feasible for ``group_alias`` to be referenced
        explicitly in many places, :meth:`_query.Query.select_entity_from`
        may be
        used at the start of the query to adapt the existing ``Group`` entity::

            q = session.query(Group).\
                select_entity_from(select_stmt.subquery()).\
                filter(Group.name == 'ed')

        Above, the generated SQL will show that the ``Group`` entity is
        adapted to our statement, even in the case of the WHERE clause:

        .. sourcecode:: sql

            SELECT anon_1.id AS anon_1_id, anon_1.name AS anon_1_name
            FROM (SELECT "group".id AS id, "group".name AS name
            FROM "group"
            WHERE "group".id = :id_1) AS anon_1
        ...
        .. sourcecode:: sql
            ...
            FROM "group", (SELECT "group".id AS id, "group".name AS name
            FROM "group"
            WHERE "group".id = :id_1) AS anon_1
            WHERE "group".name = :name_1
        ...
        :meth:`_expression.TextClause.columns` method::
            ...
            text_stmt = text("select id, name from group").columns(
                Group.id, Group.name).subquery()
            q = session.query(Group).select_entity_from(text_stmt)
        ...
        method's adaptation
        services.  Suppose
        a view ``group_view`` also returns rows from ``group``.    If
        ...
        we can use name matching to select from it::
            ...
            group_view = Table('group_view', metadata,
                              autoload_with=engine)
            group_view_alias = aliased(
                Group, group_view, adapt_on_names=True)
            q = session.query(Group).\
                select_entity_from(group_view_alias).\
                order_by(Group.name)
    ...
    def slice(self, start, stop):
        ...
        For example, ::
            ...
            session.query(Group).order_by(Group.id).slice(1, 3)
        ...
        .. sourcecode:: sql
           ...
           FROM groups ORDER BY groups.id
           LIMIT ? OFFSET ?
           (2, 1)
    ...
    def column_descriptions(self):
        ...
        Format is a list of dictionaries::
            ...
            group_alias = aliased(Group, name='group2')
            q = sess.query(Group, Group.id, group_alias)
    ...
    def exists(self):
        ...
        e.g.::
            ...
            q = session.query(Group).filter(Group.name == 'fred')
            session.query(q.exists())

        Producing SQL similar to::

            SELECT EXISTS (
                SELECT 1 FROM groups WHERE groups.name = :name_1
            ) AS anon_1

        The EXISTS construct is usually used in the WHERE clause::

            session.query(Group.id).filter(q.exists()).scalar()
    ...
    def count(self):
        ...
        expressions in conjunction with :meth:`~.Session.query`, i.e.::
            ...
            # count Group records, without
            # using a subquery.
            session.query(func.count(Group.id))

            # return count of group "id" grouped
            # by "name"
            session.query(func.count(Group.id)).\
                    group_by(Group.name)
            ...
            session.query(func.count(distinct(Group.name)))
    ...
    def delete(self, synchronize_session="evaluate"):
        ...
        E.g.::
            ...
            sess.query(Group).filter(Group.age == 25).\
    ...
    def update(self, values, synchronize_session="evaluate", update_args=None):
        ...
        E.g.::
            ...
            sess.query(Group).filter(Group.age == 25).\
                update({Group.age: Group.age - 10}, synchronize_session=False)

            sess.query(Group).filter(Group.age == 25).\
                update({"age": Group.age - 10}, synchronize_session='evaluate')
