...
This module is usually not directly visible to group applications, but
...
class QueryableAttribute(
    ...
    def info(self):
        ...
        * If the attribute is a :class:`.ColumnProperty` but is mapped to
          ...
          attribute (which should be the case, unless a group-defined SQL
...
def create_proxied_attribute(descriptor):
    """Create an QueryableAttribute / group descriptor hybrid.
...
class CollectionAttributeImpl(AttributeImpl):
    ...
    InstrumentedCollectionAttribute holds an arbitrary, group-specified
    container object (defaulting to a list) and brokers access to the
    CollectionAdapter, a "view" onto that object that presents consistent bag
    semantics to the orm layer independent of the group data implementation.
    ...
    def _default_value(self, state, dict_):
        ...
        adapter, group_data = self._initialize_collection(state)
        adapter._set_empty(group_data)
        return group_data
    ...
    def set(
        ...
        new_collection, group_data = self._initialize_collection(state)
        if _adapt:
    ...
    def set_committed_value(self, state, dict_, value):
        ...
        collection, group_data = self._initialize_collection(state)

        if value:
            collection.append_multiple_without_event(value)

        state.dict[self.key] = group_data
        ...
        if self.key in state._pending_mutations:
            ...
            state._modified_event(dict_, self, group_data, True)
    ...
    def get_collection(
        self, state, dict_, group_data=None, passive=PASSIVE_OFF
    ):
        """Retrieve the CollectionAdapter associated with the given state.

        if group_data is None, retrieves it from the state using normal
        "get()" rules, which will fire lazy callables or return the "empty"
        collection value.

        """
        if group_data is None:
            group_data = self.get(state, dict_, passive=passive)
            if group_data is PASSIVE_NO_RESULT:
                return group_data

        return group_data._sa_adapter
...
def init_state_collection(state, dict_, key):
    ...
    group_data = attr._default_value(state, dict_)
    adapter = attr.get_collection(state, dict_, group_data)
...
def flag_modified(instance, key):
    ...
    state._modified_event(dict_, impl, NO_VALUE, is_groupland=True)
...
def flag_dirty(instance):
    ...
    state, dict_ = instance_state(instance), instance_dict(instance)
    state._modified_event(dict_, None, NO_VALUE, is_groupland=True)
