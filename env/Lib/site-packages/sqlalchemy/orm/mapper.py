...
class Mapper(
    ...
    def __init__(
        ...
        :param column_prefix: A string which will be prepended
           ...
           the ``group_table`` :class:`.Table` object has columns named
           ``group_id``, ``group_name``, and ``password``::

                class Group(Base):
                    __table__ = group_table
                    __mapper_args__ = {'column_prefix':'_'}

           The above mapping will assign the ``group_id``, ``group_name``, and
           ``password`` columns to attributes named ``_group_id``,
           ``_group_name``, and ``_password`` on the mapped ``Group`` class.
        ...
        :param polymorphic_on: Specifies the column, attribute, or
          ...
          instances; this must be handled by the group,
        ...
        if self.polymorphic_on is not None:
            ...
            if self.polymorphic_on in self._columntoproperty:
                ...
                if col is None:
                    ...
                    # likely a group error if the schema.Column isn't
                    # represented somehow in either persist_selectable or
                    # with_polymorphic.   Otherwise as of 0.7.4 we
                    # just go with it and assume the group wants it
    ...
    def _adapt_inherited_property(self, key, prop, init):
        ...
        if not self.concrete:
            ...
            # mapper and we don't map this.  don't trip group-defined
    ...
    def attrs(self):
        ...
        ``Group`` class which has ``Group.name`` attribute would
    ...
    def _is_groupland_descriptor(self, assigned_name, obj):
        if isinstance(
            obj,
            (
                _MappedAttribute,
...
def configure_mappers():
    ...
    extensions or group-defined extension hooks.
