...
class JoinedLoader(AbstractRelationshipLoader):
    ...
    def setup_query(
        ...
        **kwargs
        ...
        group_defined_adapter = (
            self._init_group_defined_eager_proc(
        ...
        if group_defined_adapter is not False:
            ...
            (
                ...
                adapter,
                add_to_collection,
            ) = self._setup_query_on_group_defined_adapter(
                compile_state,
                query_entity,
                path,
                adapter,
    ...
    def _init_group_defined_eager_proc(
        ...
        # the option applies.  check if the "group_defined_eager_row_processor"
        # has been built up.
        adapter = path.get(
            compile_state.attributes, "group_defined_eager_row_processor", False
        )
        ...
        path.set(
            target_attributes,
            "group_defined_eager_row_processor",
            adapter,
        )

        return adapter

    def _setup_query_on_group_defined_adapter(
        self, context, entity, path, adapter, group_defined_adapter
    ):

        # apply some more wrapping to the "group defined adapter"
        # if we are setting up the query for SQL render.
        adapter = entity._get_entity_clauses(context)

        if adapter and group_defined_adapter:
            group_defined_adapter = group_defined_adapter.wrap(adapter)
            path.set(
                context.attributes,
                "group_defined_eager_row_processor",
                group_defined_adapter,
        ...
        return group_defined_adapter, adapter, add_to_collection
    ...
    def _create_eager_adapter(self, context, result, adapter, path, loadopt):
        ...
        if group_defined_adapter is not False:
            decorator = group_defined_adapter
            # group defined eagerloads are part of the "primary"
            # portion of the load.
