...
class InstanceState(interfaces.InspectionAttrInfo):
    ...
    def _modified_event(
        self, dict_, attr, previous, collection=False, is_groupland=False
    ):
        if attr:
            if not attr.send_modified_events:
                return
            if is_groupland and attr.key not in dict_:
                raise sa_exc.InvalidRequestError(
                    "Can't flag attribute '%s' modified; it's not present in "
                    "the object state" % attr.key
                )
            if attr.key not in self.committed_state or is_groupland:
                if collection:
