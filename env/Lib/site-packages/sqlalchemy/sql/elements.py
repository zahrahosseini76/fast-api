...
def between(expr, lower_bound, upper_bound, symmetric=False):
    ...
    E.g.::
        ...
        stmt = select(groups_table).where(between(groups_table.c.id, 5, 7))

    Would produce SQL resembling::

        SELECT id, name FROM group WHERE id BETWEEN :id_1 AND :id_2

    The :func:`.between` function is a standalone version of the
    :meth:`_expression.ColumnElement.between` method available on all
    SQL expressions, as in::

        stmt = select(groups_table).where(groups_table.c.id.between(5, 7))
...
class ClauseElement(
    ...
    def self_group(self, against=None):
        ...
        :meth:`self_group` is automatic - end-group code should never
...
class BindParameter(roles.InElementRole, ColumnElement):
    ...
    :func:`.bindparam` function, as in::
        ...
        stmt = select(groups_table).\
                    where(groups_table.c.name == bindparam('groupname'))
    ...
    def __init__(
        ...
        In order to populate the value of ``:groupname`` above, the value
        would typically be applied at execution time to a method
        like :meth:`_engine.Connection.execute`::

            result = connection.execute(stmt, groupname='wendy')
        ...
        invocation, such as::
            ...
            stmt = (groups_table.update().
                    where(group_table.c.name == bindparam('groupname')).
                    values(fullname=bindparam('fullname'))
                    )

            connection.execute(
                stmt, [{"groupname": "wendy", "fullname": "Wendy Smith"},
                       {"groupname": "jack", "fullname": "Jack Jones"},
                       ]
            )
        ...
        a comparison operation such as::
            ...
            expr = groups_table.c.name == 'Wendy'
        ...
        invoke a statement like the following::
            ...
            stmt = select(groups_table).where(groups_table.c.name == 'Wendy')
        ...
        bound placeholders based on the arguments passed, as in::
            ...
            stmt = groups_table.insert()
            result = connection.execute(stmt, name='Wendy')

        The above will produce SQL output as::

            INSERT INTO "group" (name) VALUES (%(name)s)
            {'name': 'Wendy'}
...
class TextClause(
    ...
    def _create_text(cls, text, bind=None):
        ...
        E.g.::
            ...
            t = text("SELECT * FROM groups WHERE id=:group_id")
            result = connection.execute(t, group_id=12)

        For SQL statements where a colon is required verbatim, as within
        an inline string, use a backslash to escape::

            t = text("SELECT * FROM groups WHERE name='\:groupname'")
        ...
        such as for the WHERE clause of a SELECT statement::
            ...
            s = select(groups.c.id, groups.c.name).where(text("id=:group_id"))
            result = connection.execute(s, group_id=12)
    ...
    def bindparams(self, *binds, **names_to_values):
        ...
        Given a text construct such as::
            ...
            from sqlalchemy import text
            stmt = text("SELECT id, name FROM group WHERE name=:name "
    ...
    def columns(self, *cols, **types):
        ...
        name or origin of the column in the textual SQL doesn't matter::
            ...
            stmt = text("SELECT groups.id, addresses.id, groups.id, "
                 "groups.name, addresses.email_address AS email "
                 "FROM groups JOIN addresses ON groups.id=addresses.group_id "
                 "WHERE groups.id = 1").columns(
                    Group.id,
                    Address.id,
                    Address.group_id,
                    Group.name,
            ...
            query = session.query(Group).from_statement(stmt).options(
                contains_eager(Group.addresses))
...
class BooleanClauseList(ClauseList, ColumnElement):
    ...
    def and_(cls, *clauses):
        ...
        E.g.::
            ...
            stmt = select(groups_table).where(
                            and_(
                                groups_table.c.name == 'wendy',
                                groups_table.c.enrolled == True
                            )
                        )
        ...
        clause being combined using :func:`.and_`::
            ...
            stmt = select(groups_table).\
                    where(groups_table.c.name == 'wendy').\
                    where(groups_table.c.enrolled == True)
...
class Case(ColumnElement):
    ...
    as in::
        ...
        stmt = select(groups_table).\
                    where(
                        case(
                            (groups_table.c.name == 'wendy', 'W'),
                            (groups_table.c.name == 'jack', 'J'),
                            else_='E'
                        )
                    )
...
class Extract(ColumnElement):
    ...
    def __init__(self, field, expr, **kwargs):
        ...
        Groups are reminded to consult their database documentation.
        """
...
class UnaryExpression(ColumnElement):
    ...
    def _create_nulls_first(cls, column):
        ...
        should be handled when they are encountered during ordering::
            ...
            stmt = select(groups_table).order_by(
                nulls_first(desc(groups_table.c.name)))

        The SQL expression from the above would resemble::

            SELECT id, name FROM group ORDER BY name DESC NULLS FIRST
        ...
        function version, as in::
            ...
            stmt = select(groups_table).order_by(
                groups_table.c.name.desc().nulls_first())
    ...
    def _create_nulls_last(cls, column):
        ...
        should be handled when they are encountered during ordering::
            ...
            stmt = select(groups_table).order_by(
                nulls_last(desc(groups_table.c.name)))

        The SQL expression from the above would resemble::

            SELECT id, name FROM group ORDER BY name DESC NULLS LAST
        ...
        function version, as in::
            ...
            stmt = select(groups_table).order_by(
                groups_table.c.name.desc().nulls_last())
    ...
    def _create_desc(cls, column):
        ...
        e.g.::
            ...
            stmt = select(groups_table).order_by(desc(groups_table.c.name))

        will produce SQL as::

            SELECT id, name FROM group ORDER BY name DESC
        ...
        e.g.::
            ...
            stmt = select(groups_table).order_by(groups_table.c.name.desc())
    ...
    def _create_asc(cls, column):
        ...
        e.g.::
            ...
            stmt = select(groups_table).order_by(asc(groups_table.c.name))

        will produce SQL as::

            SELECT id, name FROM group ORDER BY name ASC
        ...
        e.g.::
            ...
            stmt = select(groups_table).order_by(groups_table.c.name.asc())
    ...
    def _create_distinct(cls, expr):
        ...
        as in::
            ...
            stmt = select(func.count(distinct(groups_table.c.name)))

        The above would produce an expression resembling::

            SELECT COUNT(DISTINCT name) FROM group

        The :func:`.distinct` function is also available as a column-level
        method, e.g. :meth:`_expression.ColumnElement.distinct`, as in::

            stmt = select(func.count(groups_table.c.name.distinct()))
...
class Label(roles.LabeledColumnExprRole, ColumnElement):
    ...
    def _make_proxy(self, selectable, name=None, **kw):
        ...
        # the group would want.   See issue #6090.
...
class ColumnClause(
    ...
    :func:`_expression.column` function, as in::
        ...
        stmt = select(id, name).select_from("group")
    ...
    def __init__(self, text, type_=None, is_literal=False, _selectable=None):
        ...
        a working table construct with minimal boilerplate::
            ...
            stmt = select(group.c.description).where(group.c.name == 'wendy')
