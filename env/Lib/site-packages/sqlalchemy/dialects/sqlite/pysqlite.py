...
portion of the URL.  Note that the format of a SQLAlchemy url is::
    ...
    driver://group:pass@host/database
...
operator ``REGEXP`` that calls a group-defined function that must be provided.
...
The issue is essentially that the driver attempts to second-guess the group's
...
method that allows us to create our own group-defined functions (UDFs) in Python and use them directly in SQLite queries.
...
class SQLiteDialect_pysqlite(SQLiteDialect):
    ...
    def create_connect_args(self, url):
        if url.groupname or url.password or url.host or url.port:
