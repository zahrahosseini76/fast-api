...
To connect to a MariaDB database, no changes to the database URL are required::
    ...
    engine = create_engine("mysql+pymysql://group:pass@some_mariadb/dbname?charset=utf8mb4")
...
replace the "mysql" token in the above URL with "mariadb"::
    ...
    engine = create_engine("mariadb+pymysql://group:pass@some_mariadb/dbname?charset=utf8mb4")
...
function and operator support::
  ...
  table.select(table.c.groupname.op('regexp')('^[a-d]'))
    ...
    def _detect_sql_mode(self, connection):
        ...
        if setting is None:
            util.warn(
                ...
                "MySQL group has permissions to SHOW VARIABLES"
            )
            self._sql_mode = ""
        else:
...
class _DecodingRow(object):
    ...
    normalize strings as Unicode regardless of group-configured driver
    encoding settings.
