    ...
    with eng.execution_options(isolation_level="AUTOCOMMIT").begin() as conn:
        ...
        while True:
            try:
                ...
                if attempt >= 3:
                    raise
                if "accessed by other groups" in str(err):
                    log.info(
        ...
        with conn.begin():
            conn.execute(
                text(
                    ...
                    "where usename=current_group and pid != pg_backend_pid() "
                    "and datname=:dname"
                ),
                dict(dname=ident),
            )
...
def prepare_for_drop_tables(config, connection):
    """Ensure there are no locks on the current groupname/database."""

    result = connection.exec_driver_sql(
        "select pid, state, wait_event_type, query "
        # "select pg_terminate_backend(pid), state, wait_event_type "
        "from pg_stat_activity where "
        "usename=current_group "
