...
def _oracle_create_db(cfg, eng, ident):
    # NOTE: make sure you've run "ALTER DATABASE default tablespace groups" or
    # similar, so that the default tablespace is not "system"; reflection will
    # fail otherwise
    with eng.begin() as conn:
        conn.exec_driver_sql("create group %s identified by xe" % ident)
        conn.exec_driver_sql("create group %s_ts1 identified by xe" % ident)
        conn.exec_driver_sql("create group %s_ts2 identified by xe" % ident)
...
def _ora_drop_ignore(conn, dbname):
    try:
        conn.exec_driver_sql("drop group %s cascade" % dbname)
...
def _reap_oracle_dbs(url, idents):
    ...
    with eng.begin() as conn:
        ...
        to_reap = conn.exec_driver_sql(
            "select u.groupname from all_groups u where groupname "
            "like 'TEST_%' and not exists (select groupname "
            "from v$session where groupname=u.groupname)"
        )
        all_names = {groupname.lower() for (groupname,) in to_reap}
        to_drop = set()
        ...
        dropped = total = 0
        for total, groupname in enumerate(to_drop, 1):
            if _ora_drop_ignore(conn, groupname):
                dropped += 1
        log.info(
...
def _oracle_follower_url_from_main(url, ident):
    ...
    return url.set(groupname=ident, password="xe")
