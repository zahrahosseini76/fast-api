...
mapping which relates a ``Group`` to a ``SavingsAccount``::
...
``SavingsAccount`` entry in the list of accounts for this group.   The
in-Python getter/setter methods can treat ``accounts`` as a Python
list available on ``self``.

However, at the expression level, it's expected that the ``Group`` class will
be used in an appropriate context such that an appropriate join to
``SavingsAccount`` will be present::

    >>> print(Session().query(Group, Group.balance).
    ...       join(Group.accounts).filter(Group.balance > 5000))
    SELECT "group".id AS group_id, "group".name AS group_name,
    account.balance AS account_balance
    FROM "group" JOIN account ON "group".id = account.group_id
...
would use an outer join::
    ...
    >>> print (Session().query(Group, Group.balance).outerjoin(Group.accounts).
    ...         filter(or_(Group.balance < 5000, Group.balance == None)))
    SELECT "group".id AS group_id, "group".name AS group_name,
    account.balance AS account_balance
    FROM "group" LEFT OUTER JOIN account ON "group".id = account.group_id
    ...
    class Group(Base):
        ...
        def balance(cls):
            return select(func.sum(SavingsAccount.balance)).\
                    where(SavingsAccount.group_id==cls.id).\
                    label('total_balance')

The above recipe will give us the ``balance`` column which renders
a correlated SELECT::

    >>> print(s.query(Group).filter(Group.balance > 400))
    SELECT "group".id AS group_id, "group".name AS group_name
    FROM "group"
    WHERE (SELECT sum(account.balance) AS sum_1
    FROM account
    WHERE account.group_id = "group".id) > :param_1
