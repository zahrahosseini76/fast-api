...
asking it to reflect the schema and produce mappings::
    ...
    # engine, suppose it has two tables 'group' and 'address' set up
    ...
    session.add(Address(email_address="foo@bar.com", group=Group(name="foo")))
    session.commit()
...
our basic mapping consisting of related ``Group`` and ``Address`` classes is
...
explicit table declaration::
    ...
    metadata.reflect(engine, only=['group', 'address'])

    # ... or just define our own Table objects with it (or combine both)
    Table('group_order', metadata,
    ...
    Group, Address, Order = Base.classes.group, Base.classes.address,\
        Base.classes.group_order
...
``group`` and ``address``, we can define one or both of the classes to be used::
    ...
    # pre-declare Group for the 'group' table
    class Group(Base):
        __tablename__ = 'group'
    ...
    # but Group is the same as Base.classes.Group now

    Address = Base.classes.address

    u1 = session.query(Group).first()
    print (u1.address_collection)
...
``Group.address_collection`` and the name of the class referred to, from
...
From the above mapping, we would now have classes ``Group`` and ``Address``,
where the collection from ``Group`` to ``Address`` is called
``Group.addresses``::

    Group, Address = Base.classes.Group, Base.classes.Address

    u1 = Group(addresses=[Address(email="foo@bar.com")])
...
8. Production of the :func:`_orm.relationship` and optionally the
   ...
   function, which can be supplied by the end-group in order to augment
...
defines table metadata::
    ...
    a1 = Address(email='u1')
    a2 = Address(email='u2')
    u1 = Group(address_collection=[a1, a2])
    assert a1.group is u1

Above, given mostly complete ``Group`` and ``Address`` mappings, the
:class:`_schema.ForeignKey` which we defined on ``Address.group_id`` allowed a
bidirectional relationship pair ``Address.group`` and
``Group.address_collection`` to be generated on the mapped classes.
...
def generate_relationship(
    ...
    :param return_fn: the function that is used by default to create the
     ...
     so it is critical
     that group-defined implementations correctly differentiate between the two
