...
class AssociationProxy(interfaces.InspectionAttrInfo):
    ...
    def for_class(self, class_, obj=None):
        ...
        target class for this proxy as mapped by ``Group``::

            inspect(Group).all_orm_descriptors["keywords"].for_class(Group).target_class

        This returns an instance of :class:`.AssociationProxyInstance` that
        is specific to the ``Group`` class.   The :class:`.AssociationProxy`
...
class AssociationProxyInstance(object):
    ...
    :meth:`.AssociationProxy.for_class` method::
        ...
        proxy_state = inspect(Group).all_orm_descriptors["keywords"].for_class(Group)
