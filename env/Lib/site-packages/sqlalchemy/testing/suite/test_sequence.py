...
class HasSequenceTest(fixtures.TablesTest):
    ...
    @classmethod
    def define_tables(cls, metadata):
        Sequence("group_id_seq", metadata=metadata)
        Sequence(
            "other_seq", metadata=metadata, nomaxvalue=True, nominvalue=True
        )
        if testing.requires.schemas.enabled:
            Sequence(
                "group_id_seq", schema=config.test_schema, metadata=metadata
            )
            Sequence(
    ...
    def test_has_sequence(self, connection):
        eq_(
            inspect(connection).has_sequence("group_id_seq"),
            True,
        )

    def test_has_sequence_other_object(self, connection):
        eq_(
            inspect(connection).has_sequence("group_id_table"),
            False,
        )
    ...
    def test_has_sequence_schema(self, connection):
        eq_(
            inspect(connection).has_sequence(
                "group_id_seq", schema=config.test_schema
            ),
            True,
        )
