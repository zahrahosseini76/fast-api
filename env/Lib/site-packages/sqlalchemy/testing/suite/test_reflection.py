...
class HasTableTest(OneConnectionTablesTest):
    ...
    def temp_table_name(cls):
        return get_temp_table_name(
            config, config.db, "group_tmp_%s" % (config.ident,)
        )
    ...
    def define_temp_tables(cls, metadata):
        ...
        if (
            ...
            event.listen(
                ...
                "after_create",
                DDL(
                    "create temporary view group_tmp_v as "
                    "select * from group_tmp_%s" % config.ident
                ),
            )
            event.listen(group_tmp, "before_drop", DDL("drop view group_tmp_v"))
    ...
    def test_has_table_temp_view(self, connection):
        insp = inspect(connection)
        is_true(insp.has_table("group_tmp_v"))
...
class ComponentReflectionTest(OneConnectionTablesTest):
    ...
    def define_reflected_tables(cls, metadata, schema):
        ...
        if testing.requires.self_referential_foreign_keys.enabled:
            groups = Table(
                "groups",
                metadata,
                Column("group_id", sa.INT, primary_key=True),
                ...
                Column(
                    ...
                    sa.Integer,
                    sa.ForeignKey(
                        "%sgroups.group_id" % schema_prefix, name="group_id_fk"
                    ),
                ),
                schema=schema,
                test_needs_fk=True,
        ...
        Table(
            ...
            Column(
                "remote_group_id", sa.Integer, sa.ForeignKey(groups.c.group_id)
            ),
        ...
        if testing.requires.index_reflection.enabled:
            cls.define_index(metadata, groups)
    ...
    def define_temp_tables(cls, metadata):
        ...
        group_tmp = Table(
            ...
            sa.UniqueConstraint("name", name="group_tmp_uq_%s" % config.ident),
            sa.Index("group_tmp_ix", "foo"),
            **kw
        )
        if (
            testing.requires.view_reflection.enabled
    ...
    @classmethod
    def define_index(cls, metadata, groups):
        Index("groups_t_idx", groups.c.test1, groups.c.test2)
        Index("groups_all_idx", groups.c.group_id, groups.c.test2, groups.c.test1)

    @classmethod
    def define_views(cls, metadata, schema):
        for table_name in ("groups", "email_addresses"):
            fullname = table_name
            if schema:
    ...
    def test_get_table_names(
        ...
        if include_views:
            ...
            table_names.sort()
            answer = ["email_addresses_v", "groups_v"]
        ...
        if include_plain:
            ...
            if order_by == "foreign_key":
                answer = ["groups", "email_addresses", "dingalings"]
                eq_(table_names, answer)
            else:
                answer = ["dingalings", "email_addresses", "groups"]
                eq_(sorted(table_names), answer)
    ...
    def test_get_temp_table_names(self):
        ...
        eq_(sorted(temp_table_names), ["group_tmp_%s" % config.ident])
    ...
    def test_get_temp_view_names(self):
        ...
        eq_(sorted(temp_table_names), ["group_tmp_v"])
    ...
    def _test_get_comments(self, schema=None):
        ...
        eq_(insp.get_table_comment("groups", schema=schema), {"text": None})
    ...
    def test_get_columns(self, connection, use_views, use_schema):
        ...
        groups, addresses = (self.tables.groups, self.tables.email_addresses)
        if use_views:
            table_names = ["groups_v", "email_addresses_v"]
        else:
            table_names = ["groups", "email_addresses"]

        insp = inspect(connection)
        for table_name, table in zip(table_names, (groups, addresses)):
            schema_name = schema
    ...
    def test_get_temp_table_columns(self):
        table_name = get_temp_table_name(
            config, self.bind, "group_tmp_%s" % config.ident
        )
        group_tmp = self.tables[table_name]
        insp = inspect(self.bind)
        cols = insp.get_columns(table_name)
        self.assert_(len(cols) > 0, len(cols))

        for i, col in enumerate(group_tmp.columns):
            eq_(col.name, cols[i]["name"])
    ...
    def test_get_temp_view_columns(self):
        insp = inspect(self.bind)
        cols = insp.get_columns("group_tmp_v")
    ...
    def test_get_pk_constraint(self, connection, use_schema):
        ...
        groups_cons = insp.get_pk_constraint(groups.name, schema=schema)
        groups_pkeys = groups_cons["constrained_columns"]
        eq_(groups_pkeys, ["group_id"])
    ...
    def test_get_foreign_keys(self, connection, use_schema):
        ...
        if testing.requires.self_referential_foreign_keys.enabled:
            groups_fkeys = insp.get_foreign_keys(groups.name, schema=schema)
            fkey1 = groups_fkeys[0]

            with testing.requires.named_constraints.fail_if():
                eq_(fkey1["name"], "group_id_fk")

            eq_(fkey1["referred_schema"], expected_schema)
            eq_(fkey1["referred_table"], groups.name)
            eq_(fkey1["referred_columns"], ["group_id"])
            if testing.requires.self_referential_foreign_keys.enabled:
                eq_(fkey1["constrained_columns"], ["parent_group_id"])
        ...
        eq_(fkey1["referred_table"], groups.name)
        eq_(fkey1["referred_columns"], ["group_id"])
        eq_(fkey1["constrained_columns"], ["remote_group_id"])
    ...
    def test_get_indexes(self, connection, use_schema):
        ...
        insp = inspect(self.bind)
        indexes = insp.get_indexes("groups", schema=schema)
        expected_indexes = [
            {
                "unique": False,
            ...
            {
                "unique": False,
                "column_names": ["group_id", "test2", "test1"],
                "name": "groups_all_idx",
            },
        ]
    ...
    def test_get_temp_table_unique_constraints(self):
        insp = inspect(self.bind)
        reflected = insp.get_unique_constraints("group_tmp_%s" % config.ident)
        ...
        eq_(
            ...
            [
                {
                    "column_names": ["name"],
                    "name": "group_tmp_uq_%s" % config.ident,
                }
            ],
        )
    ...
    def test_get_temp_table_indexes(self):
        ...
        expected = [
            {"unique": False, "column_names": ["foo"], "name": "group_tmp_ix"}
        ]
        if testing.requires.index_reflects_included_columns.enabled:
            expected[0]["include_columns"] = []
        eq_(
            [idx for idx in indexes if idx["name"] == "group_tmp_ix"],
            expected,
        )
    ...
    @testing.combinations(
        ("groups", False),
        ("groups", True, testing.requires.schemas),
...
class ComponentReflectionTestExtra(fixtures.TestBase):
    ...
    def test_get_foreign_key_options(
        ...
        opts = insp.get_foreign_keys("group")[0]["options"]
        eq_(opts, expected)
