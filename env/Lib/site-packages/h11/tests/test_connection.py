...
def test_automatic_transfer_encoding_in_response() -> None:
    ...
    for group_headers in [
        ...
        group_headers = cast(List[Tuple[str, str]], group_headers)
        p = ConnectionPair()
        p.send(
            CLIENT,
            [
        ...
        p.send(
            SERVER,
            Response(status_code=200, headers=group_headers),
            expect=Response(
        ...
        assert (
            c.send(Response(status_code=200, headers=group_headers))
            == b"HTTP/1.1 200 \r\nConnection: close\r\n\r\n"
        )
        assert c.send(Data(data=b"12345")) == b"12345"
