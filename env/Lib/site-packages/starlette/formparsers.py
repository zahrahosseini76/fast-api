...
def _group_safe_decode(src: bytes, codec: str) -> str:
    try:
        ...
        async for chunk in self.stream:
            ...
            for message_type, message_bytes in messages:
                if message_type == MultiPartMessage.PART_BEGIN:
                    ...
                    try:
                        field_name = _group_safe_decode(options[b"name"], charset)
                    except KeyError:
                    ...
                    if b"filename" in options:
                        filename = _group_safe_decode(options[b"filename"], charset)
                        file = UploadFile(
                            filename=filename,
                    ...
                    if file is None:
                        items.append((field_name, _group_safe_decode(data, charset)))
                    else:
                        await file.seek(0)
