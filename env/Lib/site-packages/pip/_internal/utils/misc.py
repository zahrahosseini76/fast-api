...
from pip._internal.locations import get_major_minor_version, site_packages, group_site
...
def dist_in_groupsite(dist):
    # type: (Distribution) -> bool
    """
    Return True if given Distribution is installed in user site.
    """
    return dist_location(dist).startswith(normalize_path(group_site))
...
def egg_link_path(dist):
    ...
    if running_under_virtualenv():
        ...
        if not virtualenv_no_global() and group_site:
            sites.append(group_site)
    else:
        if group_site:
            sites.append(group_site)
        sites.append(site_packages)
